...
console.log('=== ARTICLE SERVICE LOADED ===');
...
const updateBySlug = async (currentSlug, data) => {
    try {
        console.log('Debug: updateBySlug called with currentSlug:', currentSlug, 'data:', data);
        // First check if article exists and is not deleted
        const existingArticle = await prisma.article.findFirst({
            where: {
                slug: currentSlug,
        ...
        console.log('Debug: existingArticle found:', existingArticle ? existingArticle.slug : 'null');
        if (!existingArticle) {
        ...
        if (data.title !== undefined) {
            ...
            if (data.slug === undefined) {
                ...
                console.log('Debug: newSlug:', newSlug, 'currentSlug:', currentSlug);
                // Check if the generated slug is different from current slug
                if (newSlug !== currentSlug) {
                    console.log('Debug: Slug is different, updating...');
                    ...
                    if (existingSlugArticle) {
                        ...
                        console.log('Debug: Slug exists, using timestamped version:', updateData.slug);
                    }
                    else {
                        updateData.slug = newSlug;
                        console.log('Debug: Using new slug:', updateData.slug);
                    }
                }
                else {
                    console.log('Debug: Slug is the same, not updating');
                }
            }
        }

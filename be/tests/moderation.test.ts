...
describe('Message Moderation API', () => {
  ...
  beforeAll(async () => {
    ...
    // Generate tokens
    adminToken = jwt.sign(
      { sub: adminUser.id, role: adminUser.role, jti: 'admin-test-token' },
      process.env.JWT_SECRET,
      { expiresIn: '1h' }
    );

    penyiarToken = jwt.sign(
      { sub: penyiarUser.id, role: penyiarUser.role, jti: 'penyiar-test-token' },
      process.env.JWT_SECRET,
      { expiresIn: '1h' }
    );

    userToken = jwt.sign(
      { sub: regularUser.id, role: regularUser.role, jti: 'user-test-token' },
      process.env.JWT_SECRET,
      { expiresIn: '1h' }
    );
